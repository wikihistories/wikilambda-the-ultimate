...
and of a series of failures. (Eco 1995, 19)
\end{quote}
...
aims to transform Wikipedia from a \emph{multi}lingual encyclopaeida
...
repeatedly failed'' (Vrandečić 2020, 180; 2021, 41). Abstract Wikipedia
is the latest in a long line of attempts to create a ``perfect'' or
``universal'' language---to be more precise, it is the latest in a long
line of ``polygraphies,'' or universal writing systems (Eco 1995, chap.
9). Since every prior attempt at polygraphy has failed, it is hardly
surprising that Abstract Wikipedia risks failing also!
...
\emph{two}. Work on the template language that would enable people to
contribute articles to Abstract Wikipedia had barely begun. Instead, the
project had devoted nearly all its effort to the creation of a brand new
programming language in which the 300-6,000 ``renderers'' for Abstract
Wikipedia would be written. This programming language would make
Abstract Wikipedia possible, by allowing universal access to
programming:
...
program. ({``Wikifunctions:{Vision}''} 2023)
\end{quote}
...
Wikimedia Endowment ({``Annual {Report} 2022-23''} 2023; Foundation
2023). A prototype of the project's ``ad-hoc'' and ``questionable''
programming system is already available at
\href{https://wikifunctions.org}{wikifunctions.org}. How did the project
get into this situation, and what does its future hold?
...
\section{Architecture of Abstract
...
decision, driven by convenience. (Vrandečić 2020, 182)
\end{quote}
...
Egyptian Arabic and Romanian, to rely on machine translation and bots to
...
Vrandečić (2020, 178), then the more ``comprehensive, current, and
...
San Francisco. Ideally, it shouldn't matter what languages you read, you
should be able to access San Francisco's current ranking by population
among Californian cities ({``Abstract {Wikipedia}/{Architecture}''}
2024), the name of its current Mayor (Vrandečić 2020, 176--77), and the
fact that it is currently the cultural, commercial and financial centre
of Northern California (Vrandečić 2018, 2021). Since these facts are not
currently available to readers of all languages on Wikipedia, argues
Vrandečić (2020, 182), the Wikipedia project has thus far failed to
allow ``access to the sum of all knowledge to every single reader, no
...
a manually-written article ({``Abstract {Wikipedia}/{Architecture}''}
2024; {``Abstract {Wikipedia}/{Components}''} 2024).
...
of transforming the abstract content into readable Croatian text. The
reason there are multiple ``function evaluators'' we will see in
...
Wikilambda Extension. Source: {``Extension:{WikiLambda}''} (2024). By
jdforrester (Wikimedia Foundation)
...
principle of the Wikipedia project (2020, 181). To justify this
...
Croatian: Serbian Wikipedia, Croation Wikipedia, and Serbo-Croatian
...
dialects of Croation are often larger than the differences between
standard Croation and standard Serbian'' (2020, 183). Then in the next
sentence, he complains that Croatian Wikipedia has fascist leanings.
There is a subtle contradiction between these arguments, which defeats
Vrandečić's point. He uses the term ``language'' in two senses. When he
argues that there is very little ``linguistic'' difference between
standard Serbian and standard Croation, he is referring to what
linguists call language\textsubscript{1}, or language defined in terms
of its vocabulary and syntax. When he complains that Croation Wikipedia
...
With Culture,'' so he can establish that it doesn't matter what language
is used to express a fact. But in trying to prove his point, he presents
a powerful example that language\textsubscript{2} \emph{does} strongly
align with culture.
...
doesn't matter whether you state this in English, Serbo-Croation,
...
syntax that make statements intelligible. Persistent metaphorical
structures such as \textsc{the world is a map} and \textsc{human manners
are a garden} frame experience in a certain way. ``San Francisco is the
cultural centre of Northern California'' is not a neutral fact. It is
only relevant or meaningful if you already divide the human world into
geographically distinctive regions, and believe that certain spots in
those regions are more culturally fertile. If you speak English or
another European language, you probably \emph{do} divide the world up in
this way. if you speak another language, then Vrandečić's example may
not even count as a fact.
...
\item
  ...
  meaning of the Content {[}\ldots{]} ({``Abstract
  {Wikipedia}/{Architecture}''} 2024)
\end{enumerate}
\end{quote}
...
the processes of ``construction'' and ``rendering,'' as the
\textsc{language is a conduit} metaphor requires. Within the overall
project, Wikilambda's purpose is to provide the constructors that
package the content, and the renderers that unpack it at the other end
of the conduit. In their attempt to escape language and universalise
knowledge, Vrandečić and his team have plunged into English, the very
language they have been most concerned to escape.
...
\section{Wikilambda the Perfect}\label{sec-perfection}

\begin{quote}
\textbf{Wikifunctions is not a programming language, nor is trying to
evangelise a particular language} ({``Wikifunctions:{What}
{Wikifunctions} Is Not''} 2024)
\end{quote}

Umberto Eco (1995, 73) distinguishes two kinds of ideal language: the
...
universally accessible (Vee 2017, 43--46).
...
languages like JavsScript use English words, they force budding
programmers to ``learn English first'' before they learn to program,
which is unfair ({``Wikifunctions:{Vision}''} 2023). To solve this
problem, Wikilambda does not use words to denote parts of a computation.
Instead, each part of the computation is assigned a \texttt{Z}-number or
\texttt{Z}-key in the Wikifunctions database. When a person visits a
function in the Wikifunctions interface, they are presented with a
translation of these \texttt{Z}-numbers and \texttt{Z}-keys into their
preferred language. If English is the preferred language in your
browser, then when you visit function
...
\textsc{conduit} metaphor to prove that it doesn't matter what language
...
a programming language'' ({``Wikifunctions:{What} {Wikifunctions} Is
Not''} 2024). This is the sense in which Wikilambda is a ``perfect''
language. If a perfect language mirrors the true nature of objects, then
Wikilambda mirrors the true nature of computation. What is the true
nature of computation? Wikilambda's answer is implicit in the name of
the software (Wikilambda) and the name of the online database where it
runs (Wikifunctions). Computation can be boiled down to a single kind of
primitive element, the \emph{function} (or a \emph{lambda}, to use the
Greek terminology of computer science pioneer Alonzo Church (1932)).
Complex computations can be created by \emph{composing} functions
together. Wikilambda allows users to define \emph{functions} and
\emph{compose} them together ``and that's it'' ({``Wikifunctions:{What}
{Wikifunctions} Is Not''} 2024). Wikilambda is not a ``language,''
infected with English vocabulary, which comes between the user and the
computation. It is an ``an abstract syntax tree,'' which allows the user
to modify the structure of the computation directly (Blanton et al.
2022).
...
(Massaro et al. 2024b). As a result, the team have become involved in
...
like LISP in JSON'' ({``Wikifunctions:{Function} Model''} 2024).
Wikilambda is indeed very much like LISP, but in their drive for extreme
simplicity, the Wikilambda developers have overlooked key elements that
make LISP usable in practice, such as tail recursion, the REPL, partial
compilation, garbage collection, specialised data structures, special
forms, and quasiquotation (see generally Sussman and Steele Jr. 1975;
Steele Jr. 1977; Steele Jr. and Sussman 1978; Abelson, Sussman, and
Sussman 1996). Simply put, even just to provide access to the ``abstract
syntax tree,'' Wikilambda will need to become a lot more powerful; to
become useable enough to support the complex software for Abstract
Wikipedia, it will need to become more powerful still. And through all
this, the English-speaking Wikilambda developers will be telling their
own ``one-voice story'' in the code they write for the orchestrator.
...
debated in Ndebele---but then any programmers who don't already read and
write Ndebele would be hampered from contributing code.
...
behave the way they do in spoken or written language'' (Marino 2020,
145--47). For example, the function \texttt{mapcar} in the Scheme
...
human languages, with their own culture and idioms (Blackwell 2017, 37).
If a programming language contains an English word, then it is a
...
metaphors, styles, ideals. It doesn't much matter if
...
\section{Abstraction and Metaphor in the Orchestrator}\label{sec-code}
...
A programming language, argue Abelson, Sussman, and Sussman (1996, 607),
is ``a coherent family of abstractions erected on the machine
language.'' The language itself provides a family of abstractions to the
programmer: using JavaScript, I can create an \texttt{object} or
interact with a \texttt{document} without worrying about the particular
sequence of CPU instructions that makes an \texttt{object} or a
\texttt{document} behave as it does. The language does not merely
\emph{provide} abstractions, however---it is \emph{made} of
abstractions. Someone has to write the code that defines what a
JavaScript \emph{object} or \emph{document} is. The code that implements
a programming language will contain many abstractions, such as
\emph{expression}, \emph{symbol} or \emph{token}, which are used to
describe the language being implemented. Perplexingly, many programming
languages are implemented \emph{in themselves}. The C compiler, for
example, is itself written in C. C provides a coherent family of
abstractions, which is then used to describe and implement C, which
provides a coherent family of abstractions, which is then used to
...
``function orchestrator'' (Massaro et al. 2024b) the piece of JavaScript
code that implements the Wikilambda language. What abstractions have the
Wikilambda developers invented to describe their new language? What can
these abstractions tell us about the nature and intent of their
project?\footnote{The function orchestrator (and its companion, the
  evaluator), are both in active development. I have analysed them at
  the time I cloned the code: that is commit
...
(2014, 223), Alexander Galloway (2012, 64) and Denis Tenen (2017, 32)
have argued that code is illusory. In reality, software is made up of
``varying electric potentials'' or ``electromagnetic charges'' in the
computer hardware. The code supervenes on these electrical realities: at
best, code ``signifies'' or ``substitutes for'' the electrons in the
CPU; at worst, it ``obfuscates'' them. If this is so, then there is
little reason to interpret the abstractions in source code. Why should
we concern ourselves with the code, rather than with the material
reality it signifies, substitutes or obfuscates?
...
about or conceptualising the behaviour of the machine. Some of these
abstractions are so familiar that we have forgotten they are
metaphors.\footnote{They have become ``metaphors we live by'' (Lakoff
  and Johnson 1980).} It may seem that computers literally have a
``memory,'' but this is only because we rely on an underlying metaphor:
\textsc{the computer is a brain}. The Victorian computer pioneer Charles
Babbage used a different metaphor: \textsc{the computer is a factory}.
His proposed Analytical Engine had a ``storehouse'' for data, rather
than a ``memory'' (Lovelace and Menabrea 2021). Thus even the hardware
of the computer is abstract and metaphorical. Even the ``varying
electrical charges'' of Kittler and his followers are abstractions over
underlying quantum effects, which are themselves abstractions developed
by human mathematicians. At no depth does the metaphorical tower of
abstraction descend to a literal foundation.
...
virtue of'' their ability to shape computations. In the same manner that
alcohol has the virtue of intoxication, programming abstractions have
the virtue of functional specification. They are metaphors that say
\emph{what the machine is supposed to do}. If the machine does what it
is supposed to, we say the abstraction is ``correct.'' If the machine
misbehaves, we say the abstraction has a ``bug.'' If the machine does
what it is supposed to, but we realise we want it to do something
different, we say that we need a ``new abstraction.'' The electrical
charges in the hardware are not the same thing as the software, because
the electrical charges can be wrong.\footnote{The pattern of electrical
  charges will also differ for every computer chip. Indeed, due to the
  abstraction of chip architecture, two chips may share the same machine
  code, but implement that machine code using different circuitry. Taken
  at is extreme, the ``information hiding'' view implies that a piece of
...
software's fault---or not, if the wrongness is caused by electronic
interference or quantum fluctuations in the circuitry. The abstractions
in source code are virtually the real thing, and scholars of CCS should
treat them as such.\footnote{I hope I have established the plausibility
  of this account. It raises several live problems: Is there a
  difference between signs and symbols (Langer 1957)? What is the
  relation between metaphors and abstractions (Lakoff 1993; Ragg 2006;
  Jamrozik et al. 2016)? What is the meaning of terms such as reality,
  possibility, actuality and virtuality (Bergson 1908; Deleuze 1998,
  2014)? CCS must address these issues for its own sake, but also
  promises to enrich these venerable discussions with new data in a
  fascinating new kind of symbolism.}
...
(Massaro et al. 2024a). Listing~\ref{lst-immaHeadOut} vividly
...
src/Executor.js, in Massaro et al. (2024a)}
...
written with a mixture of bruality and tenderness that bespeaks the
...
``implementations'' in the Wikifunctions database. It is this
\texttt{ImplementationSelector} that most clearly virtualises the
``orchestration'' metaphor. Normally, a programming language will have
\emph{just one} way of doing each action: one function for addition, one
for integer division, one for instantiating an array, and so on. If
there are two ways of doing something, it would normally be up to the
\emph{programmer} to decide: perhaps there are two division routines,
one that is fast and approximate and one that is slow but exact, and the
programmer can select which one is appropriate for their task. The
Wikilambda language is different, because there may be many ways of
performing each operation, and it is the orchestrator's job rather than
the programmer's to choose between them. At the time of writing, for
example, there are three implementations of the ``add Integers''
function
...
The composer writes a piano score describing the structure of the
symphony, while the orchestrator decides which instruments should fill
out which parts.
...
2024b). But I would suggest the code reveals a deeper pattern than mere
...
that perfection must imply universality. (Eco 1995, 100)
\end{quote}
...
These LLMs rely on vast concealed datasets. Wikifunctions draws its data
openly from public Wikimedia databases. These LLMs generate text using
opaque algorithms that even their designers cannot understand.
Wikilambda makes every part of every algorithm available to anyone. In
short, Wikilambda is \emph{contestable}. If you ask ChatGPT to generate
an article on a topic, the only way to contest its algorithm is to click
👍 or 👎 (Crawford and Gillespie 2016). If you are unhappy with an
article generated by Abstract Wikipedia, you will be able to: change the
...
most popular programming metaphors: \textsc{the world is Tolkein's
...
(Silicon Valley), and use it to dominate the world. The tragic
contradiction of the Abstract Wikipedia project is that it is itself a
``\,`One Ring' solution.'' A group of Anglophone engineers in the heart
of Mordor (California) are forging one language that all the world's
writers should write, and another language that all the world's
...
separating them'' (Frame 2008, 117). It is the grand ambition of Denny
Vrandečić and his hackers to learn this chemical process, which has
evaded so many linguistic alchemists before them.
...
{``Abstract {Wikipedia}/{Architecture}.''} 2024. \emph{Wikimedia
Meta-Wiki}.
...
{``Abstract {Wikipedia}/{Components}.''} 2024. \emph{Wikipedia
Meta-Wiki}.
...
{``Annual {Report} 2022-23.''} 2023. San Francisco: Wikimedia Endowment.
...
Avieson, Bunty. 2022. {``Two {Wikipedias} in {Bhutan}: Problems and
Solutions for Knowledge Equity in the Digital Age.''} \emph{Asian
Journal of Communication} 32 (5): 399--416.
\url{https://doi.org/10.1080/01292986.2021.1937248}.

\bibitem[\citeproctext]{ref-bayer_wikimedia_2023}
Bayer, Tilman. 2023. {``Wikimedia {Foundation}'s {Abstract} {Wikipedia}
Project 'at Substantial Risk of Failure'.''} \emph{The Signpost},
January.
...
Bergson, Henri. 1908. \emph{Essai Sur Les Données Immédiates de La
Conscience}. 6th ed. Paris: Alcan.

\bibitem[\citeproctext]{ref-blackwell_6000_2017}
Blackwell, Alan F. 2017. {``6,000 {Years} of {Programming} {Language}
{Design}: {A} {Meditation} on {Eco}'s {Perfect} {Language}.''} In
\emph{Conversations {Around} {Semiotic} {Engineering}}, edited by Simone
Diniz Junqueira Barbosa and Karin Breitman, 31--39. Cham: Springer
International Publishing.
\url{https://doi.org/10.1007/978-3-319-56291-9_5}.

\bibitem[\citeproctext]{ref-blackwell_moral_2024}
---------. 2024. \emph{Moral {Codes}: {Designing} {Alternatives} to
{AI}}. The MIT Press.
\url{https://doi.org/10.7551/mitpress/14872.001.0001}.

\bibitem[\citeproctext]{ref-blanton_abstract_2022}
Blanton, Cai, Cory Massaro, David Martin, Denny Vrandečić, Genoveva
Galarza Heredero, James Forrester, Julia Kieserman, and Stef Dunlap.
2022. {``Abstract {Wikipedia}/{Google}.org {Fellows} Evaluation -
{Answer}.''} \emph{Wikimedia Meta-Wiki}.
...
Carrillo, Natalia, and Sergio Martínez. 2023. {``Scientific {Inquiry}:
{From} {Metaphors} to {Abstraction}.''} \emph{Perspectives on Science}
31 (2): 233--61. \url{https://doi.org/10.1162/posc_a_00571}.

\bibitem[\citeproctext]{ref-church_set_1932}
Church, Alonzo. 1932. {``A {Set} of {Postulates} for the {Foundation} of
{Logic}.''} \emph{Annals of Mathematics} 33 (2): 346--66.
\url{https://doi.org/10.2307/1968337}.

\bibitem[\citeproctext]{ref-colburn_abstraction_2007}
Colburn, Timothy, and Gary Shute. 2007. {``Abstraction in {Computer}
{Science}.''} \emph{Minds and Machines: Journal for Artificial
Intelligence, Philosophy and Cognitive Science} 17 (2): 169--84.
...
\emph{New Media \& Society} 18 (3): 410--28.
\url{https://doi.org/10.1177/1461444814543163}.

\bibitem[\citeproctext]{ref-deleuze_bergsonisme_1998}
Deleuze, Gilles. 1998. \emph{Le Bergsonisme}. 2. ed. Paris: Presses
Univ. de France.

\bibitem[\citeproctext]{ref-deleuze_difference_2014}
---------. 2014. \emph{Difference and {Repetition}}. Translated by Paul
Patton. 2nd edition. London; New York: Bloomsbury Academic.

\bibitem[\citeproctext]{ref-dillon_what_2023}
Dillon, Sarah, and Jennifer Schaffer-Goddard. 2023. {``What {AI}
Researchers Read: The Role of Literature in Artificial Intelligence
Research.''} \emph{Interdisciplinary Science Reviews} 48 (1): 15--42.
\url{https://doi.org/10.1080/03080188.2022.2079214}.

\bibitem[\citeproctext]{ref-dubrow_cosmopolitan_2018}
Dubrow, Jennifer. 2018. \emph{Cosmopolitan {Dreams}: {The} {Making} of
{Modern} {Urdu} {Literary} {Culture} in {Colonial} {South} {Asia}}.
Honolulu: University of Hawaii Press.

\bibitem[\citeproctext]{ref-eco_search_1995}
Eco, Umberto. 1995. \emph{The Search for the Perfect Language}. Making
of {Europe}. Oxford, UK ; Cambridge, Mass., USA: Blackwell.

\bibitem[\citeproctext]{ref-noauthor_extensionwikilambda_2024}
{``Extension:{WikiLambda}.''} 2024. \emph{MediaWiki}.
...
Ford, Heather. 2022. \emph{Writing the Revolution: {Wikipedia} and the
Survival of Facts in the Digital Age}. Cambridge, Massachusetts: The MIT
Press.

\bibitem[\citeproctext]{ref-ford_wikidata_2023}
Ford, Heather, and Andrew Iliadis. 2023. {``Wikidata as {Semantic}
{Infrastructure}: {Knowledge} {Representation}, {Data} {Labor}, and
{Truth} in a {More}-{Than}-{Technical} {Project}.''} \emph{Social Media
+ Society} 9 (3): 20563051231195552.
\url{https://doi.org/10.1177/20563051231195552}.

\bibitem[\citeproctext]{ref-ford_how_2024}
Ford, Heather, Francesca Sidoti, Michael Falk, Tamson Pietsch, and Tom
Byers. 2024. {``How {Australian} Places Are Represented on
{Wikipedia}.''} 2. University of Technology, Sydney.
\url{https://doi.org/10.5281/zenodo.13910503}.

\bibitem[\citeproctext]{ref-foundation_first_2023}
Foundation, Wikimedia. 2023. {``First Grants Announced from the
{Wikimedia} {Endowment} to Support Technical Innovation Across
{Wikipedia} and {Wikimedia} Projects.''} \emph{Wikimedia Foundation}.
...
Frame, Janet. 2008. \emph{Storms Will Tell: Selected Poems}. Tarset,
Northumberland: Bloodaxe Books.

\bibitem[\citeproctext]{ref-galloway_interface_2012}
Galloway, Alexander R. 2012. \emph{The Interface Effect}. Cambridge, UK
; Malden, MA: Polity.

\bibitem[\citeproctext]{ref-gnatiuk_uneven_2021}
Gnatiuk, Oleksiy, and Victoria Glybovets. 2021. {``Uneven Geographies in
the Various Language Editions of {Wikipedia}: The Case of {Ukrainian}
Cities.''} \emph{Hungarian Geographical Bulletin} 70 (3): 249--66.
\url{https://doi.org/10.15201/hungeobull.70.3.4}.

\bibitem[\citeproctext]{ref-hakala_negotiating_2016}
Hakala, Walter. 2016. \emph{Negotiating {Languages}: {Urdu}, {Hindi},
and the {Definition} of {Modern} {South} {Asia}}. New York, UNITED
...
Hoffmann, Dorothea. 2019. {``Restrictions on the {Usage} of {Spatial}
{Frames} of {Reference} in {Location} and {Orientation} {Descriptions}:
{Evidence} from {Three} {Australian} {Languages}.''} \emph{Australian
Journal of Linguistics} 39 (1): 1--31.
\url{https://doi.org/10.1080/07268602.2019.1542927}.

\bibitem[\citeproctext]{ref-hua_how_2023}
Hua, Minh, and Rita Raley. 2023. {``How to {Do} {Things} with {Deep}
{Learning} {Code}.''} \emph{Digital Humanities Quarterly} 17 (2).
\url{https://www.digitalhumanities.org/dhq/vol/17/2/000684/000684.html}.

\bibitem[\citeproctext]{ref-jamrozik_metaphor_2016}
Jamrozik, Anja, Marguerite McQuire, Eileen R. Cardillo, and Anjan
Chatterjee. 2016. {``Metaphor: {Bridging} Embodiment to Abstraction.''}
\emph{Psychonomic Bulletin \& Review} 23 (4): 1080--89.
\url{https://doi.org/10.3758/s13423-015-0861-0}.

\bibitem[\citeproctext]{ref-kittler_there_2014}
Kittler, Friedrich A. 2014. {``There Is No Software.''} In \emph{The
{Truth} of the {Technological} {World}: {Essays} on the {Genealogy} of
{Presence}}, translated by Erik Butler, 219--29. New York, UNITED
...
\emph{Metaphor and {Thought}}, edited by Andrew Ortony, 2nd ed.,
202--51. Cambridge University Press.
\url{https://doi.org/10.1017/CBO9781139173865.013}.

\bibitem[\citeproctext]{ref-lakoff_metaphors_1980}
Lakoff, George, and Mark Johnson. 1980. \emph{Metaphors We {Live} {By}}.
Chicago; London: The University of Chicago Press.

\bibitem[\citeproctext]{ref-langer_philosophy_1957}
Langer, Susanne K. 1957. \emph{Philosophy in a New Key; a Study in the
Symbolism of Reason, Rite, and Art.} {[}3d ed.{]}. Cambridge: Harvard
University Press.

\bibitem[\citeproctext]{ref-livneh_abstract_2022}
Livneh, Ori, Ariel Gitman, Ari Assaf, and Mary Yang. 2022. {``Abstract
{Wikipedia}/{Google}.org {Fellows} Evaluation.''} \emph{Wikipedia
Meta-Wiki}.
...
Lovelace, Ada, and L. F. Menabrea. 2021. {``Sketch of the {Analytical}
{Engine} (1843).''} In \emph{Ideas {That} {Created} the {Future}},
edited by Harry Lewis, 9--26. The MIT Press.
\url{https://doi.org/10.7551/mitpress/12274.003.0005}.

\bibitem[\citeproctext]{ref-marino_critical_2020}
Marino, Mark C. 2020. \emph{Critical {Code} {Studies}}. The MIT Press.
\url{https://doi.org/10.7551/mitpress/12122.001.0001}.

\bibitem[\citeproctext]{ref-massaro_function-evaluator_2024}
Massaro, Cory, Denny Vrandečić, Daphne Smit, David Martin, Ecarg,
Genoveva Galarza, Jeena Huneidi, James Forrester, Stef Dunlap, and
Vaughn Walters. 2024a. {``Function-Evaluator.''}
...
Ragg, Edward. 2006. {``Pragmatic {Abstraction} Vs. {Metaphor}:
{Stevens}' "{The} {Pure} {Good} of {Theory}" and "{Macbeth}".''}
\emph{The Wallace Stevens Journal} 30 (1): 5--29.
\url{https://www.jstor.org/stable/44884979}.

\bibitem[\citeproctext]{ref-reddy_conduit_1993}
Reddy, Michael J. 1993. {``The Conduit Metaphor: {A} Case of Frame
Conflict in Our Language about Language.''} In \emph{Metaphor and
{Thought}}, edited by Andrew Ortony, 2nd ed., 164--201. Cambridge
University Press. \url{https://doi.org/10.1017/CBO9781139173865.012}.

\bibitem[\citeproctext]{ref-rountree_nonsense_2023}
Rountree, Barry, and William Condee. 2023. {``Nonsense {Code}: {A}
{Nonmaterial} {Performance}.''} \emph{Digital Humanities Quarterly} 17
(2).
\url{https://www.digitalhumanities.org/dhq/vol/17/2/000702/000702.html}.

\bibitem[\citeproctext]{ref-steele_jr_debunking_1977}
Steele Jr., Guy Lewis. 1977. {``Debunking the "Expensive Procedure Call"
Myth; or, Procedure Call Implementations Considered Harmful; or, Lambda:
The Ultimate Goto.''} 443. Cambridge, Mass: Massachusetts Institute of
Technology. \url{https://apps.dtic.mil/sti/pdfs/ADA062381.pdf}.

\bibitem[\citeproctext]{ref-steele_jr_art_1978}
Steele Jr., Guy Lewis, and Gerald Jay Sussman. 1978. {``The {Art} of the
{Interpreter}; or, {The} {Modularity} {Complex} ({Parts} {Zero}, {One},
and {Two}).''} 453. Cambridge, Mass: Massachusetts Institute of
Technology.
\url{https://dspace.mit.edu/bitstream/handle/1721.1/6094/AIM-453.pdf}.

\bibitem[\citeproctext]{ref-sussman_scheme_1975}
Sussman, Gerald Jay, and Guy Lewis Steele Jr. 1975. {``Scheme: {An}
Interpreter for Extended Lambda Calculus.''} 349. Cambridge, Mass:
Massachusetts Institute of Technology.
\url{https://dspace.mit.edu/bitstream/handle/1721.1/5794/AIM-349.pdf}.

\bibitem[\citeproctext]{ref-swift_gullivers_2005}
Swift, Jonathan. 2005. \emph{Gulliver's {Travels}}. Edited by Claude
Rawson. Oxford: OUP.

\bibitem[\citeproctext]{ref-tenen_plain_2017}
Tenen, Dennis. 2017. \emph{Plain Text: The Poetics of Computation}.
Stanford, California: Stanford University Press.

\bibitem[\citeproctext]{ref-wa_thiongo_decolonising_1986}
Thiong'o, Ngugi wa. 1986. \emph{Decolonising the {Mind}: {The}
{Politics} of {Language} in {African} {Literature}}. Woodbridge: Boydell
\& Brewer, Limited.

\bibitem[\citeproctext]{ref-tkacz_wikipedia_2015}
Tkacz, Nathaniel. 2015. \emph{Wikipedia and the Politics of Openness}.
Chicago ; London: University of Chicago Press.

\bibitem[\citeproctext]{ref-vee_coding_2017}
Vee, Annette. 2017. \emph{Coding {Literacy}: {How} {Computer}
{Programming} {Is} {Changing} {Writing}}. The MIT Press.
\url{https://doi.org/10.7551/mitpress/10655.001.0001}.

\bibitem[\citeproctext]{ref-vrandecic_capturing_2018}
Vrandečić, Denny. 2018. {``Capturing Meaning: {Toward} an Abstract
{Wikipedia}.''} In \emph{International {Semantic} {Web} {Conference}
2018 - {Outrageous} {Ideas} {Track}}. Monterey, CA.
\url{http://ceur-ws.org/Vol-2180/}.

\bibitem[\citeproctext]{ref-vrandecic_collaborating_2020}
---------. 2020. {``Collaborating on the {Sum} of {All} {Knowledge}
{Across} {Languages},''} October.
\url{https://doi.org/10.7551/mitpress/12366.003.0016}.

\bibitem[\citeproctext]{ref-vrandecic_building_2021}
---------. 2021. {``Building a Multilingual {Wikipedia}.''}
\emph{Communications of the ACM} 64 (4): 38--41.
\url{https://doi.org/10.1145/3425778}.

\bibitem[\citeproctext]{ref-wark_hacker_2004}
Wark, McKenzie. 2004. \emph{A {Hacker} {Manifesto}}. Cambridge, Mass:
Harvard University Press. \url{https://doi.org/10.4159/9780674044845}.

\bibitem[\citeproctext]{ref-noauthor_wikifunctionsfunction_2024}
{``Wikifunctions:{Function} Model.''} 2024. \emph{Wikifunctions}.
...
{``Wikifunctions:{Vision}.''} 2023. \emph{Wikifunctions}.
...
{``Wikifunctions:{What} {Wikifunctions} Is Not.''} 2024.
\emph{Wikifunctions}.
